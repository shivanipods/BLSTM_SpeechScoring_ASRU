...
def lstm_regression(options,it):
    ...
    # for itr in xrange(1):
        ...
        if ((itr + 1) % save_interval_in_iters) == 0 or is_last_iter:
            ...
            file_name = check_point_basename + '_' \
                        + str(itr + 1) + '_' +  str(it) + '_' +'%.2f' %(sum(val_cost_list)/len(val_cost_list))+'.model'
...
if __name__ == '__main__':
    n_dim_range = [1024,2046]
    best = numpy.float32(1000)
    it = 0;
    validation_mse =[]
    for n_dim in n_dim_range:
        options = dict()
        ...
        mse = lstm_regression(options,it)
        it =it+1
        if mse < best:
            param = options
            best = mse
    ...
    print best
    with open('model/param_n_dim.pkl', 'wb') as f:
            pkl.dump(param, f)
            pkl.dump(best)
